...
class Product extends BaseModel {
    ...
    protected $fillable = ['name_ru', 'name_ua', 'section', 'insection', 'cat', 'price', 'bprice',
        'color', 'gallery_color', 'size', 'gallery', 'gallery_hover', 'data_ru','data_ua', 'vendor', 'related', 'to_buy', 'keywords', 'description', 'stock'];
    ...
    public static function updateRecord($id, $input) {
        if(isset($input['bprice']) && $input['bprice'] == '') unset($input['bprice']);
        try {
            if(isset($input['section'])) $input['section'] = intval($input['section']);
            if(isset($input['cat'])) $input['cat'] = intval($input['cat']);
            if (!$id) {
    ...
    public static function getInSection($section, $select = ['*'], $with = [], $offset = 0, $limit = 0){
        ...
        $query = self::where('insection', "LIKE", '%"'.$section.'"%')->select($select)->with($with)->offset($offset);
        if($limit){
            $query->limit($limit);
        }
        return $query->get();
